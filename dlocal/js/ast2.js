...
var jscode = fs.readFileSync('D:\\huangsheng\\self\\js_workspace\\node_workspace\\js_parse\\dlocal\\js\\source_detail.js', "utf-8", function (err, data) {
    if (err) {
        console.log(err);
    } else {
        code = data.toString();
...
function regExpPattern2ABC(path) {
    let node = path.node;
    if (!node.pattern) return;
    let value = node.extra.raw;
    value = "'" + value + "' ";
    path.replaceWith(types.RegExpLiteral(eval(value)));
    path.stop()
}

// 删除var T0SS = T0SS;
function delVarT0SSeqT0SS(path) {
    let node = path.node;
    let {name} = node.id;
    if (name !== 'T0SS') return;
    if (!node.init) return;
    let init_name = node.init.name;
    if (name !== null && init_name === 'T0SS') {
        path.remove();
    }
}

function renameH(path) {
    let node = path.node;
    let {name} = node.id;
    if (!node.init) return;
    let init_name = node.init.name;
    if (name !== null && init_name === 'T0SS') {
        path.scope.rename(name, 'T0SS');
    }
}
...
function replaceT0SSValue(path) {
    ...
    if (!callee.property.name) return;
    let callFuncName = callee.property.name;
    let {arguments} = node;

...
// 获取data的加密
function getData(slide, token, _end) {
    var traceData = [];
    for (var index = 0;index < slide.length;index ++) {
        traceData.push(_Nnn(token, slide[index] + ""));
    }
    var n = sample(traceData, 50);
    // n = traceData;
    var r = get_cb(_Nnn(token, parseInt(_end, 10) / 320 * 100 + ""));
    var data = JSON.stringify({
        d: get_cb(n.join(":")),
        m: "",
        p: r,
        ext: get_cb(_Nnn(token, 1 + "," + traceData.length))
    });
    return data;
}
...
app.post('/encrypt_guiji', function (req,res) {
    var body = req.body;
    var distance = body.distance;
    var token = body.token;
    var slide = body.slide;
    slide = JSON.parse(slide);
    // var make_slide = require('./make_slide');
    // var slide = make_slide(distance);
    var data = getData(slide, token, distance);
    console.log(data);
    res.send(urlEncode(data));
})

app.listen(3005, ()=>console.log("轨迹服务已开启"));
